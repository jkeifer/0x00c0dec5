    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm }}>
      {variables.map((v) => {
        ...
        const rangeNote = stats && Number.isFinite(stats.min) && Number.isFinite(stats.max)
          ...
          : '';
        // The cast's own damage only — a lossy codec step reports on its own
        // badge in the Codecs section (codec-lossy-{variable}-{index}), not here.
        const isLossy = Boolean(stats?.isLossy);
