...
- `variable-source-{index}` — per-variable Schema editor source dropdown: first option `Custom (generated)` (value `custom`), then `CURATED_VARIABLES` filtered to the active data model and grouped by dataset via `<optgroup>`, option value `${datasetId}/${variableName}`. Selecting a curated source dispatches `UPDATE_VARIABLE` with the `source` ref (or clears it for `custom`); the reducer then pulls the catalog `logicalType` (deep copy) and defaults `typeAssignment` from the natural dtype, and locks `logicalType` (only) while `source` is set. `variable-source-attribution-{index}` — the tertiary informational attribution hint line shown while a row has a source (the catalog `attribution`; writes nothing to metadata).
- (TASK 4 will rewrite the following `dataset-*` paragraph — it describes the removed schema-wide dataset mechanism. Curated data is now per-variable via `Variable.source`; there is no `dataset-select`/`dataset-attribution`/`dataset-loading`/`dataset-error`, no `state.dataset`, no `isDatasetVariable`, no `APPLY_DATASET`/`SET_DATASET_CUSTOM`.)
