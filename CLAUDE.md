...
- `table-cell-{variable}-{index}` — individual table cells (`{index}` is the absolute element/row index)
- TableView virtualizes rows via `useRowVirtual` (`src/components/viewers/useRowVirtual.ts`), a small fixed-row-height virtualizer with **scroll compression** — NOT `@tanstack/react-virtual`. A virtual list sizes one spacer div at `rowCount × ROW_HEIGHT` (24px); browsers clamp any element to ~2²⁴ px, so past ~699K rows a plain list silently truncates. Below `MAX_SCROLL_PX` (15,000,000, headroom under 2²⁴) it's an ordinary 1:1 virtual list; above it the spacer is capped at `MAX_SCROLL_PX` and the visible row range + row `top`s are derived from the scroll fraction, so a **single native scrollbar reaches every row** (no windowing, no jump/overview controls — those were removed). Tradeoff: past the cap each scrollbar pixel spans multiple rows, so fine scrolling near the ends is coarse (cross-pane hover auto-scroll via `scrollToIndex` still lands exactly). The sticky column header shares the scroll container; the hook's `headerPx` keeps rows clear of it. HexView still uses its own per-section `@tanstack/react-virtual` + windowing (`hex-overview`/`hex-offset-input` below) — its multi-section `scrollMargin` architecture wasn't migrated to `useRowVirtual`.
...
- `codec-add-{variable}` — the per-pipeline "+ Add codec" dropdown (`{variable}` is the variable name, or `chunk` for the row-mode chunk pipeline); target this rather than heuristics over a row's `<select>`s — a preset-shipped step's params can render their own selects (Scale/Offset's sourceDtype/targetDtype)
...
- `codec-lossy-{variable}-{index}` — per-step lossy badge, shown when that step's `CodecStepStats` (threaded from the worker payload) report `clipped > 0 || rounded > 0` — reads `"lossy: {clipped} clipped, {rounded} rounded"`. Since the typeAssignment shrink this is how a lossy transform (Quantize/Bit Round/Scale-Offset) surfaces its damage per step, in place of the old `VariableStats.isLossy` badge next to Type Assignment.
- `codec-row-inactive-note-{variable}` — row-mode-only note rendered on the first step of a field pipeline's inactive remainder (per CLAUDE.md pitfall 4's `splitStructuredPrefix`): "inactive in row mode — output past this step has no per-element structure to interleave".
...
- The custom-entries list is headed by a static "Custom / Overrides" label (no testid — plain text) that names the override-wins semantics for the whole list rather than per row. Override-wins: a custom entry whose key matches an auto-collected key replaces the auto entry's value in place (position preserved) rather than being renamed away — the old `user_`-prefix rename and its `metadata-key-collision-warning-{i}` warning border are both gone, and so is the former per-row `metadata-key-override-note-{i}` note (it didn't land in a sensible place as the list grew; the list-level label replaced it). Lying to the reader (wrong `shape`, fake `codec_pipelines`) is an intended lesson, not a mistake to warn against. The empty-key warning border on a blank custom key is unaffected. The "+ Entry" button sits at the **bottom** of the list (after the rows), and the Metadata Format selector (JSON/Binary, labeled "Metadata Format") sits up with the include-group selectors, not at the bottom of the section.
...
- `type-assign-range-{variableId}` — the Type Assignment section's observed-range note per variable ("range min … max", `TypeAssignConfig.tsx`), read from the live worker `VariableStats.min`/`max` (the same stats that drive the lossless/lossy indicator beside it) so it reflects the actual current shape/crop, not a static manifest value. Shown only when stats exist and min/max are finite (all-NaN variables report NaN and the note is skipped). Purpose: give the `storageDtype` choice a concrete target (e.g. "0 … 2469" fits uint16 but not int8) — a scale factor, if the values need one, is now chosen on a Scale/Offset codec step in the pipeline, not here.
...
- `grid-diff-toggle` — GridView's per-pane "diff" button, shown **only on the Read stage** (i.e. when the pane has `diffValues`, the originals to diff the reconstruction against). It's a per-session, per-pane knob (`useState`, not persisted AppState — like `grid-stretch-select`), **not** a global. Diff coloring commandeers the grid's value→color ramp for a diverging Δ scale, so it's a genuine either/or that needs a toggle, and it's mutually exclusive with the stretch picker (which hides while diff is on). This replaces the former global `state.ui.showDiff` + the sidebar Read section's "Show differences from original" radio, both gone. **TableView has no such toggle**: its diff cues (a per-column summary line + amber cells + hover Δ) are purely additive — they replace nothing — so the Read-stage table *always* renders diffs when it has `diffValues`. StagePane derives `diffValues` (ungated) for the Read stage and passes it to both viewers; TableView renders it always, GridView gates it behind `grid-diff-toggle`.
- `hex-overview` — HexView's FileMapStrip, shown for windowed sections above `WINDOWED_SECTION_ROWS` (262,144 rows); click-to-jump. `hex-offset-input` — the paired offset-jump text input (accepts hex like `0x100000`, Enter to jump). Both live in `HexWindowControls`, which is **sticky** (`position: sticky`, `topOffset` = `HEADER_HEIGHT` when a multi-file section header is also shown, else `0`) so scrolling the window doesn't lose them. The jump accepts **any** offset, not just line-aligned ones: the byte snaps to its containing row via `Math.floor(offset / bytesPerRow)`, then `goToRow` windows-if-needed and scrolls to center that row (shared with cross-pane hover's imperative `scrollToRow`).
...
3. **One mechanism for dtype flow — `outputDtypeFor`, params-aware — covers everything, including what used to be Type Assignment's scale/offset/bit-rounding.** Since the codec-unification refactor, `TypeAssignment` is a bare cast: `{ storageDtype: DtypeKey }`, nothing else. Quantize, Bit Round, and Scale/Offset are ordinary `category: 'transform'` codecs living in `CODEC_REGISTRY` (`src/engine/codecs.ts`) like every other codec — there is no separate per-variable lossy-conversion step anymore. Each codec declares `sizeEffect: 'preserving' | 'fixed-ratio' | 'variable'` (`src/types/codecs.ts`), which drives size math via `encodedByteLength` (`src/engine/codecs.ts`): `preserving` (most codecs, including Quantize/Bit Round — same dtype, same width in and out), `fixed-ratio` (Scale/Offset — a deterministic per-element width change computed from geometry, e.g. float32→int16 is 1/2, needing no chunk index to stay offset-computable), `variable` (every entropy codec — data-dependent, `encodedByteLength` returns `null`). `outputDtypeFor(codec, inputDtype, params)` is the single source of truth for the dtype-flow rule and is **params-aware** — Scale/Offset's `outputDtype(inputDtype, params)` reads `params.targetDtype` directly, since its output width isn't derivable from the input dtype alone. Every step's `encode()` input dtype is the previous step's `outputDtype` (or the variable's `typeAssignment.storageDtype` for the first step); call `outputDtypeFor` rather than re-deriving the rule locally (`reverseCodecPipeline` and `isPipelineLossy` both used to, and both silently went stale when the rule changed). The default rule (absent an explicit `outputDtype` function): **`uint8` for anything whose output has no elements in it** — every entropy codec, *and* every codec declaring a `traceMode` (Byte Shuffle's byte planes, Bit Shuffle's bit planes). Value-rewriting codecs (Delta, Zigzag, Quantize, Bit Round) preserve the input dtype; Scale/Offset overrides to its param-selected `targetDtype`. A codec that moves bytes must not report the pre-move dtype: that lie propagates into the next step's `elementSize`, the ⚠ warnings, and the dtype label on the step. What the Encoded *pane* draws for a positional slot is a different question with a different answer — `encodedChunkMeta().slotDtype`, the pre-shuffle element a naive reader would decode there. Reversal is a single phase now, not two: `reverseCodecPipeline` (`src/engine/decode.ts`) walks the whole pipeline backward — transform steps included, e.g. Scale/Offset's `decode()` reads its own `sourceDtype`/`scale`/`offset` params to un-scale — and the **handoff invariant still holds**: the fully-reversed pipeline's output dtype must equal the variable's `typeAssignment.storageDtype` (there is no separate `reverseTypeAssignment` step anymore — it was deleted along with the old `type_assignments` metadata entry). Test the dtype flow explicitly at both boundaries: within the codec pipeline itself (including transform steps), and at that final handoff.

4. **Interleaving mode switches — row mode now runs a per-variable prefix, not nothing.** When switching from column to row interleaving, each field pipeline's *maximal element-structured prefix* still runs per-variable before interleaving — only the remainder becomes inactive. `splitStructuredPrefix(steps)` (`src/engine/codecs.ts`) walks a field pipeline and splits it at the first step that isn't `isElementStructured` (a step is element-structured when it has no `traceMode` and `sizeEffect !== 'variable'` — i.e. it still leaves one identifiable slot per element for interleaving to operate on). The prefix runs per-variable exactly as it would in column mode; the remainder — plus the shared `chunkPipeline` — runs on the interleaved stream. `SET_INTERLEAVING` still never touches `fieldPipelines`/`chunkPipeline` state; switching back to column reactivates the full field pipeline unchanged. `CodecPipelineEditor` renders the inactive remainder dimmed with a `codec-row-inactive-note-{variable}` note on its first step. Because a fixed-ratio step (Scale/Offset) can end a prefix at different widths per variable, `CodecSection.tsx` computes each variable's **post-prefix** dtype (`pipelineOutputDtype` over just the prefix) and gates the mixed-dtype warning (`codec-mixed-dtype-warning`) on *those* — not on the variables' raw `storageDtype`s, which was the pre-fix behavior and false-positived whenever prefixes converged to the same width from different starting dtypes.
