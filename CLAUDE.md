...
- `type-assign-range-{variableId}` — the Type Assignment section's observed-range note per variable ("range min … max", `TypeAssignConfig.tsx`), read from the live worker `VariableStats.min`/`max` (the same stats that drive the lossless/lossy indicator beside it) so it reflects the actual current shape/crop, not a static manifest value. Shown only when stats exist and min/max are finite (all-NaN variables report NaN and the note is skipped). Purpose: give the storageDtype/scale choice a concrete target (e.g. "0 … 2469" → ×10 still fits int16).
